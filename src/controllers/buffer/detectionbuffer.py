...
class DetectionBuffer:
    def __init__(self) -> None:
        ...
        self._frame: DetectionPacket | None = None
        self._version = 0
        self._closed = False

    def publish(self, frame: DetectionPacket) -> None:
    ...
    def wait_next(self, pre_version: int, timeout: float) -> tuple[int, DetectionPacket | None]:
        with self._condition:
    ...
    def snapshot(self) -> tuple[int, DetectionPacket | None]:
        with self._condition:
            return self._version, self._frame
